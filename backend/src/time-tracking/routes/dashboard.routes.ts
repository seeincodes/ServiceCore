    ...
    try {
      ...
      for (let i = 0; i < days; i++) {
        ...
        results.push({
          ...
          absent: totalDrivers - presentCount,
    ...
    try {
      ...
      const results = rows.map((r: any) => {
        ...
        return {
          ...
          otHours,
          daysWorked,
          avgDaily: daysWorked > 0 ? Math.round((totalHours / daysWorked) * 100) / 100 : 0,
        };
      });
...
// POST /manager/reports/export — export report as CSV
router.post(
  '/reports/export',
  authenticate,
  authorize('manager', 'org_admin'),
  async (req: Request, res: Response) => {
    try {
      ...
      if (!type || !startDate || !endDate) {
        sendError(res, 'type, startDate, and endDate are required', 400);
        return;
      }

      if (format !== 'csv') {
        sendError(res, 'Only csv format is supported', 400);
        return;
      }
      ...
      if (type === 'attendance') {
        ...
        csvContent = 'Date,Total Drivers,Present,Absent,Avg Hours\n';
        ...
        for (let i = 0; i < diffDays; i++) {
          ...
          const presentCount = Number(presentResult?.count || 0);
          const avgHours = Math.round((Number(avgResult?.avg_hours) || 0) * 100) / 100;
          csvContent += `${dateStr},${totalDrivers},${presentCount},${totalDrivers - presentCount},${avgHours}\n`;
        }
      } else if (type === 'overtime') {
        csvContent = 'Week Ending,Total OT Hours,Drivers With OT,Avg OT Per Driver\n';
        ...
        const rows = await db('clock_entries')
          .join('users', 'clock_entries.user_id', 'users.id')
          .where({ 'clock_entries.org_id': user.orgId })
          .where('clock_in', '>=', start.toISOString())
          .where('clock_in', '<=', end.toISOString())
          .whereNotNull('clock_out')
          .groupBy('clock_entries.user_id', 'users.first_name', 'users.last_name')
          .select(
            'clock_entries.user_id',
        ...
        for (const r of rows) {
          ...
          const daysWorked = Number(r.days_worked);
          const otHours = Math.max(0, Math.round((totalHours - 40) * 100) / 100);
          const avgDaily = daysWorked > 0 ? Math.round((totalHours / daysWorked) * 100) / 100 : 0;
        ...
        for (const r of rows) {
          const totalHours = Math.round(Number(r.total_hours) * 100) / 100;
          const name = String(r.project_name).replace(/,/g, ' ');
          csvContent += `${r.project_id},${name},${r.project_code},${totalHours},${Number(r.driver_count)}\n`;
        }
      } else {
        sendError(res, 'Invalid report type', 400);
        return;
      }
...
// GET /manager/driver/:userId/notes
router.get(
  '/driver/:userId/notes',
  authenticate,
  authorize('manager', 'org_admin'),
  async (req: Request, res: Response) => {
    try {
      ...
      const rows = await db('audit_log')
        .where({ org_id: user.orgId, user_id: userId, action: 'manager_note', entity_type: 'user' })
        .orderBy('created_at', 'desc');
      ...
      const message = err instanceof Error ? err.message : 'Failed to get notes';
      sendError(res, message);
    }
  },
);

// POST /manager/driver/:userId/notes
router.post(
  '/driver/:userId/notes',
  authenticate,
  authorize('manager', 'org_admin'),
  async (req: Request, res: Response) => {
    try {
      ...
      // Get manager name
      const manager = await db('users').where({ id: user.id }).first();
      const createdByName = manager ? `${manager.first_name} ${manager.last_name}` : 'Unknown';

      const [note] = await db('audit_log')
        .insert({
          org_id: user.orgId,
          user_id: userId,
          action: 'manager_note',
          entity_type: 'user',
          details: JSON.stringify({ text, createdBy: user.id, createdByName }),
        })
        .returning('*');

      const details = typeof note.details === 'string' ? JSON.parse(note.details) : note.details;
      sendSuccess(
        res,
        {
          note: {
...
// DELETE /manager/driver/:userId/notes/:noteId
router.delete(
  '/driver/:userId/notes/:noteId',
  authenticate,
  authorize('manager', 'org_admin'),
  async (req: Request, res: Response) => {
    try {
...
// GET /manager/driver/:userId/performance?weeks=4
router.get(
  '/driver/:userId/performance',
  authenticate,
  authorize('manager', 'org_admin'),
  async (req: Request, res: Response) => {
    try {
      ...
      for (let i = 0; i < weeks; i++) {
        ...
        for (const entry of entries) {
          ...
          if (!daysSet.has(dateKey)) {
            daysSet.add(dateKey);
            // Use the first clock-in of the day for avg start time
            totalStartHour += clockIn.getHours() + clockIn.getMinutes() / 60;
            if (clockIn.getHours() >= 8) {
              lateArrivals++;
            }
          }
        }
        ...
        const avgStartTime =
          daysWorked > 0 ? Math.round((totalStartHour / daysWorked) * 100) / 100 : 0;
      ...
      sendSuccess(res, { weeks: results });
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Performance summary failed';
      sendError(res, message);
    }
  },
);
...
// POST /manager/notifications/:id/read
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const updated = await db('notifications')
        .where({ id: notificationId, org_id: user.orgId, user_id: user.id })
...
// POST /manager/notifications/read-all
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      sendSuccess(res, { status: 'all read' });
    } catch (err: unknown) {
